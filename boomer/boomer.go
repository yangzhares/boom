...
func (b *Boomer) newRequest(urlSuffix string) (*http.Request, error) {
	url := strings.TrimSuffix(b.URL, "/")
	req, err := http.NewRequest(b.Method, url+urlSuffix, nil)
	//fmt.Printf("URL: %v\n", url+urlSuffix)
	if err != nil {
		return nil, err
	}
...
func (b *Boomer) makeRequest(c *http.Client) {
	...
	request, _ := b.newRequest("")
	s := time.Now()
	resp, err := c.Do(cloneRequest(request, b.RequestBody))
	if err == nil {
